...
const calculateExercises = (trainingHoursPerDay: number[], target: number): Result => {
    ...
    const average = trainingHoursPerDay.reduce((prevValue, currentValue) => (prevValue + currentValue)) / trainingHoursPerDay.length;
...
try {
    const { trainingHoursPerDay, target } = parseArguments(process.argv);
